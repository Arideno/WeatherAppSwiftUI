...
final class OpenWeatherService {
    ...
    func getWeather(for city: String) -> AnyPublisher<WeatherData?, Never> {
        guard let url = URL(string: "http://api.openweathermap.org/data/2.5/weather?q=\(city)&units=metric&appid=\(APP_ID)") else { return AnyPublisher(Empty<WeatherData?, Never>()) }
